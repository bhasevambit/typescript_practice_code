const messsage: string = "Hello, world! from TypeScript practice garage";
...
const inputFilepath: string = path.join(fileDir, "../uhyo.txt");
console.log("ReadFilePath = " + inputFilepath);

function count(sentence: string, searchWord: string): number {
  let count = 0;
  let idx = 0;
...
try {
  ...
  const searchWord: string = "uhyo";
  const result: number = count(sentence, searchWord);
